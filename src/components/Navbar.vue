<template>
  <nav>
    <div class="nav-row">
      ...
      >{{result}}</span>
      <svg style="width:24px;height:24px" viewBox="0 0 24 24" @click="settingVisible = !settingVisible">
    ...
    <div class="nav-row" v-if="settingVisible">
      <span>
        <label for="rowsInput">Rows</label>
        <input type="number" min="10" id="rowsInput">
      </span>
      <span>
        <label for="columnsInput">Columns</label>
        <input type="number" min="10" id="columnsInput">
      </span>
      <span>
        <label for="minesInput">Mines</label>
        <input type="number" min="10" id="minesInput">
      </span>
    </div>
  </nav>
</template>
